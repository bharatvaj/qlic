...
/* const char* saved_grant_token = ""; */
			...
			/* qlic_state.FIELD = "HI";\ //__LINE__##FIELD->text_value; \ */
...
char *json_access_code_transformer(char* str) {
	...
	qlic_state.access_token = get_val(str, "access_token", oauth_tokens, oauth_tok_count);
	qlic_state.refresh_token = get_val(str, "refresh_token", oauth_tokens, oauth_tok_count);
...
char* start_oauth_server(QlicConfig* qlic_config) {
	/* int err = -1; */
