...
static int qlic_usage() {
	fputs("usage: qlic [-vla] [-s msg |-r <chat_id> [chat_id..]]\n", stderr);
	return -1;
}
...
int qlic_send_text_msg(const qstr access_token, const qstr msg) {
	...
	for (size_t i = 0; i < chat_ids_len; i++) {
...
int main(int argc, char *argv[]) {
	...
	char config_jstr[QLIC_FP_READ_SIZE];
	jsmntok_t config_tokens[32];
	jsmn_init(&config_json);

	qstr home_path = qstrnew(getenv("HOME"));
	qstr config_path = qstrnew(qlic_env_default_vars[0]);
	...
	size_t config_jstr_len = fread(config_jstr, 1, QLIC_FP_READ_SIZE, config_fp);
	if (!config_jstr_len) {
		perror(__FILE__);
		return -1;
	}
	fclose(config_fp);
	size_t config_tok_count = jsmn_parse(&config_json, config_jstr, config_jstr_len, config_tokens, 32);

	/* Validate json */
	if (config_tok_count < 2) {
		inform("config.json is invalid. Note: file size cannot be more than %d\n", QLIC_FP_READ_SIZE);
	}
	...
	qlic_config.client_id = json_get_val(config_jstr, "client_id", config_tokens, config_tok_count);
	qlic_config.client_secret = json_get_val(config_jstr, "client_secret", config_tokens, config_tok_count);
